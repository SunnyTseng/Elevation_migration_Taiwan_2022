...
data_preparation_predictors <- function(dir_tiff = here("data",
                                                        "raw" , 
  ...
  data_qualified <- read_csv(dir_eBird)
  
  observation_coors <- data_qualified %>%
  ...
  value_df <- data_qualified %>% select(global_unique_identifier)
